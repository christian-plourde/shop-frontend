...
class ProductImageCarousel extends React.Component {
  //accept an array of image urls of any size as props
  constructor(props) {
    super(props);
    this.state = {
      currentImageIndex: 0,
  ...
  previousSlide() {
    //modulo operation to decrement current image's index, can loop from index 0 to max index
    let totalImage = this.state.imageUrl.length;
    let prevImageIndex =
      (this.state.currentImageIndex + totalImage - 1) % totalImage;
    this.setState({ currentImageIndex: prevImageIndex });
  }

  nextSlide() {
    //modulo operation to increment current image's index, can loop from max index to index 0
    let nextImageIndex =
