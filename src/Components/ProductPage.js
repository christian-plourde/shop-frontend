...
class ProductPage extends React.Component {
  ...
  componentDidMount() {
    ...
    const productSource = localhost
      ? `http://localhost:8081/shop-backend/php/product.php?product=${product_id}`
    ...
    const reviewSource = localhost //acquiring the review array associated with the received prop's ID
      ? `http://localhost:8081/shop-backend/php/reviews.php?review=${product_id}`
  ...
  render() {
    ...
    if (!isLoaded) {
      ...
      const reviewComponents = this.state.productReviews.map(
        (reviewItems, index) => (
          <div>
            {this.state.isAdmin ? (
              <DeleteButton
      ...
      if (this.state.product.images != undefined) {
        imageSources = this.state.product.images.map(
          imageUrl =>
            `https://shop-354.herokuapp.com/ressources/img/${imageUrl}`
        ); //generating a product image url array with randomly inserted Shrek, ISS and Donkey(from Shrek)
      }
            ...
            <div class="container">
              <div class="right-column">
                <div class="product-description">
                  ...
                  <p>{this.state.product.descriptionText}</p>
                </div>
                ...
                <span class="product-price">
                  <span>${this.state.product.productPrice}</span>
