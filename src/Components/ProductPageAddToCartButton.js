...
class AddToCartButton extends React.Component {
  //adds product to cart once and redirect to cart page, because otherwise navbar doesn't synchronize
  constructor(props) {
    super(props);
  ...
  changeHandler() {
    ...
    if (cart == null) {
      //create cart item and set cart quanity as 1, if localStorage doesn't contain cart
      localStorage.setItem("cart", JSON.stringify(productToAdd));
      localStorage.setItem("cartQuantity", "1");
    } else {
      // update cart item and cart quantity if it exists
      ...
      let cartProducts = cartProductsJSON.map(productJSON =>
      ...
      for (let product of cartProducts) {
        //if product already exists among them, increment product quantity
      ...
      if (!alreadyInCart) {
        //if product is not in cart, add product to it
      ...
      cartProducts.map(
        product =>
          (newCartProductsJSON =
            newCartProductsJSON + "|" + JSON.stringify(product))
      );
  ...
  render() {
    if (localStorage.getItem("logged_in_user") == null) return <br />;
    else {
      if (this.state.redirect == false)
        return (
          <button onClick={this.changeHandler} class="cartAdd">
            Add to Cart
          </button>
        );
